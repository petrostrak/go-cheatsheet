...
// data are present and not blank. If the fields fail this check, add the
...
func (f *Form) MaxLength(field string, d int) {
	...
	if utf8.RuneCountInString(value) > d {
		f.Errors.Add(field, fmt.Sprintf("This field is too long (maximum is %d characters\n)", d))
	}
}

// Implement a PermittedValues method to check that a specific field in the form
// matches one of a set of specific permitted values. If the check fails then
...
// contains a minimun number of characters. If the check fails then add the
// appropriate message to the form errors.
...
// matches a regular expression. If the check fails then add the appropriate message
// to the form errors.
